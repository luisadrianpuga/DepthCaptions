...
def _fit_font(
    font_path: str | None, word: str, target_width: int
...
def render_word_on_frame(
    pil_frame: Image.Image, word: str, font_path: str | None, cfg: Config
) -> Image.Image:
    fw, fh = pil_frame.size
    font = _fit_font(font_path, word, int(fw * cfg.text_fill_width))
    ...
    color_rgba = (*cfg.text_color, alpha)
...
def process_video(input_path: str, output_path: str, cfg: Config = DEFAULT) -> str:
    ...
    try:
        ...
        while True:
            ...
            word = active_word_at(words, t)
            captioned = render_word_on_frame(pil_frame, word, font_path, cfg) if word else pil_frame
